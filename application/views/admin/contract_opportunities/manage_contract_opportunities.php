...
<div id="wrapper">
    <div class="content" id="vueApp">
        <div class="row">
            <div class="col-md-12">
                <div
                    class="contract-opportunities-overview tw-mb-6<?= $isKanBan ? ' hide' : ''; ?>">
                    <h4 class="tw-my-0 tw-font-bold tw-text-xl tw-mb-2">
                        <?= _l('contract_opportunities'); ?>
                    </h4>
                ...
                <div class="_buttons tw-mb-2">
                    <div class="tw-flex tw-items-center tw-justify-between tw-space-x-2 rtl:tw-space-x-reverse">
                        <div class="tw-flex tw-items-center tw-space-x-1 rtl:tw-space-x-reverse">
                            <a href="#" onclick="init_contract_opportunity(); return false;" class="btn btn-primary">
                            ...
                            <a href="<?= admin_url('leads/switch_kanban/' . $switch_kanban); ?>"
                                ...
                                data-title="<?= $switch_kanban == 1 ? _l('contract_opportunities_switch_to_kanban') : _l('switch_to_list_view'); ?>">
                            ...
                            <?php if (is_admin() || get_option('allow_non_admin_members_to_import_contract_opportunities') == '1') { ?>
                            <a href="<?= admin_url('leads/import'); ?>"
                                class="hidden-xs btn btn-default">
                                <i class="fa-solid fa-upload tw-mr-1"></i>
                                <?= _l('import_contract_opportunities'); ?>
                            </a>
                            <?php } ?>
                        </div>
                        <div>
                            <?php if ($this->session->userdata('contract_opportunities_kanban_view') == 'true') { ?>
                            <div class="contract-opportunities-search">
                                <div data-toggle="tooltip" data-placement="top"
                                    data-title="<?= _l('search_by_tags'); ?>">
                                    <?= render_input('search', '', '', 'search', ['data-name' => 'search', 'onkeyup' => 'contract_opportunities_kanban();', 'placeholder' => _l('contract_opportunities_search')], [], 'no-margin') ?>
                                </div>
                            </div>
                            <?php } else { ?>
                            <div class="tw-inline">
                            ...
                            <?= form_hidden('sort', (get_option('default_contract_opportunities_kanban_sort') != '' ? get_option('default_contract_opportunities_kanban_sort_type') : '')); ?>
                        </div>
                    </div>
                </div>
                <div
                        ...
                        if ($isKanBan) { ?>
                            <div class="active kan-ban-tab tw-mt-4" id="kan-ban-tab" style="overflow:auto;">
                                <div class="kanban-leads-sort">
                                    <span
                                        class="bold"><?= _l('contract_opportunities_sort_by'); ?>:
                                    </span>
                                    <a href="#" onclick="contract_opportunities_kanban_sort('dateadded'); return false"
                                        class="dateadded">
                                        <?php if (get_option('default_contract_opportunities_kanban_sort') == 'dateadded') {
                                            echo '<i class="kanban-sort-icon fa fa-sort-amount-' . strtolower(get_option('default_contract_opportunities_kanban_sort_type')) . '"></i> ';
                                        } ?><?= _l('contract_opportunities_sort_by_datecreated'); ?>
                                    </a>
                                    |
                                    <a href="#" onclick="contract_opportunities_kanban_sort('leadorder');return false;"
                                        class="leadorder">
                                        <?php if (get_option('default_contract_opportunities_kanban_sort') == 'leadorder') {
                                            echo '<i class="kanban-sort-icon fa fa-sort-amount-' . strtolower(get_option('default_contract_opportunities_kanban_sort_type')) . '"></i> ';
                                        } ?><?= _l('contract_opportunities_sort_by_kanban_order'); ?>
                                    </a>
                                    |
                                    <a href="#" onclick="contract_opportunities_kanban_sort('lastcontact');return false;"
                                        class="lastcontact">
                                        <?php if (get_option('default_contract_opportunities_kanban_sort') == 'lastcontact') {
                                            echo '<i class="kanban-sort-icon fa fa-sort-amount-' . strtolower(get_option('default_contract_opportunities_kanban_sort_type')) . '"></i> ';
                                        } ?><?= _l('contract_opportunities_sort_by_lastcontact'); ?>
                                    </a>
                                </div>
                                <div class="row">
                                    <div class="container-fluid contract-opportunities-kan-ban">
                                        <div id="kan-ban"></div>
                                    </div>
                                </div>
                            </div>
                            <?php } else { ?>
                            <div class="row" id="contract-opportunities-table">
                                <div class="col-md-12">
                                                    ...
                                                    <div id="bulk_change">
                                                        <div class="form-group">
                                                            <div class="checkbox checkbox-primary checkbox-inline">
                                                                <input type="checkbox" name="contract_opportunities_bulk_mark_lost"
                                                                    id="contract_opportunities_bulk_mark_lost" value="1">
                                                                <label for="contract_opportunities_bulk_mark_lost">
                                                                    <?= _l('contract_opportunity_mark_as_lost'); ?>
                                                                </label>
                                                            </div>
                                                        </div>
                                ...
                                echo render_datetime_input('leads_bulk_last_contact', 'contract_opportunities_dt_last_contact');
                                echo render_select('assign_to_leads_bulk', $staff, ['staffid', ['firstname', 'lastname']], 'contract_opportunities_dt_assigned');
                                ?>
                                                        <div class="form-group">
                                                        ...
                                                        <div class="form-group no-mbot">
                                                            <div class="radio radio-primary radio-inline">
                                                                ...
                                                                <label for="leads_bulk_public">
                                                                    <?= _l('contract_opportunity_public'); ?>
                                                                </label>
                                                            </div>
                                                            <div class="radio radio-primary radio-inline">
                                                                <input type="radio" name="leads_bulk_visibility"
                              ...
                              $table_data    = [];
                                $_table_data = [
                                    ...
                                    [
                                        'name'     => _l('contract_opportunities_dt_name'),
                                ...
                                $_table_data[] = [
                                    'name'     => _l('contract_opportunity_company'),
                                    'th_attrs' => ['class' => 'toggleable', 'id' => 'th-company'],
                                ];
                                $_table_data[] = [
                                    'name'     => _l('contract_opportunities_dt_email'),
                                    'th_attrs' => ['class' => 'toggleable', 'id' => 'th-email'],
                                ];
                                $_table_data[] = [
                                    'name'     => _l('contract_opportunities_dt_phonenumber'),
                                    'th_attrs' => ['class' => 'toggleable', 'id' => 'th-phone'],
                                ];
                                $_table_data[] = [
                                    'name'     => _l('contract_opportunities_dt_value'),
                                ...
                                $_table_data[] = [
                                    'name'     => _l('contract_opportunities_dt_assigned'),
                                    'th_attrs' => ['class' => 'toggleable', 'id' => 'th-assigned'],
                                ];
                                $_table_data[] = [
                                    'name'     => _l('contract_opportunities_dt_status'),
                                    'th_attrs' => ['class' => 'toggleable', 'id' => 'th-status'],
                                ];
                                $_table_data[] = [
                                    'name'     => _l('contract_opportunity_source'),
                                    'th_attrs' => ['class' => 'toggleable', 'id' => 'th-source'],
                                ];
                                $_table_data[] = [
                                    'name'     => _l('contract_opportunities_dt_last_contact'),
                                    'th_attrs' => ['class' => 'toggleable', 'id' => 'th-last-contact'],
                                ];
                                $_table_data[] = [
                                    'name'     => _l('contract_opportunities_dt_datecreated'),
...
<script>
    ...
    $(function() {
        contract_opportunities_kanban();
        $('#contract_opportunities_bulk_mark_lost').on('change', function() {
        ...
        $('#move_to_status_leads_bulk').on('change', function() {
            if ($(this).selectpicker('val') != '') {
                $('#contract_opportunities_bulk_mark_lost').prop('disabled', true);
                $('#contract_opportunities_bulk_mark_lost').prop('checked', false);
            } else {
                $('#contract_opportunities_bulk_mark_lost').prop('disabled', false);
            }
        });
    });
</script>
